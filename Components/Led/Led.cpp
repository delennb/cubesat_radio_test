...
namespace Components {
  ...
  Led::Led(const char* const compName) : LedComponentBase(compName), state(Fw::On::OFF), 
                                          transitions(0), count(0), blinking(false) 
  {
  ...
  void Led::BLINKING_ON_OFF_cmdHandler(const FwOpcodeType opCode, const U32 cmdSeq, Fw::On on_off) {
    ...
    if(!on_off.isValid()) {
      ...
      // Trigger the SetBlinkingState event
      this->log_ACTIVITY_HI_SetBlinkingState(on_off);

      // Record the state in the telemetry channel
